...
try:    
    ...
    print('Grabbing and processing the spectrum data from corner-turner output snap block (channel = %i, offset = %i chans (%i bytes))...' % (opts.offset, channel_location, offset_in_bytes), end=' ')
    sys.stdout.flush()
    ...
    for f in range(0, len(up32[0]) / sword_per_fchan):
        ...
        if opts.verbose: print('%i(%i)' % (freq_index, fchan_lookup[freq_index]), end=' ')
        sys.stdout.flush()
